...
자리요(ZariYo)는 관리자(매장 오너)가 2D GUI 드래그 앤 드롭 캔버스를 통해 매장 레이아웃과 좌석 배치를 자유롭게 설계하고, 손님은 휴대폰 간편 인증을 바탕으로 실시간 2D 공간 예약, 무인 주문 및 결제, 직원 호출 서비스를 통합하여 이용할 수 있는 차세대 풀스택 공간 제어 및 스마트 오더 솔루션입니다.
...
- **무가입 4단계 릴레이 순차 UX**: 휴대폰 번호 기반의 방문 로그 인증 및 위치 기반 매장 검색 릴레이 파이프라인을 구축하여 손님의 이용 접근성을 극대화했습니다.

---

## 2. 주요 핵심 기능 상세

### 위치 기반 지오코딩 (Geocoding) 지점 매핑 엔진
...
### WebSocket STOMP 실시간 양방향 알림
- **관제 대시보드 및 주방 KDS 연동**: 주문 접수, 조리 시작, 서빙 완료, 직원 호출 상태 변경 이벤트를 웹소켓 브로드캐스팅 채널(/sub/orders, /sub/calls)로 전송하여 관제판에 실시간 팝업 및 상태 변화를 반영합니다.
...
- **Realtime Networking**: @stomp/stompjs, sockjs-client

### Backend & Infrastructure
- **Core Framework**: Java 17, Spring Boot 3.x, Spring Data JPA, Spring Security
- **Database Layer**: MySQL 8.0 (Relational Data), Redis 6.2 (Cache & Redisson Distributed Lock)
- **Security & Auth**: JWT (JSON Web Token) Access/Refresh Token
- **API Documentation**: OpenAPI 3 (Springdoc Swagger UI)
- **Virtualization**: Docker Compose (MySQL 3306, Redis 6379)

---

## 4. 백엔드 7대 코어 도메인 API 명세
...
├── work.md                  # 전체 마일스톤 및 개발 일지 (105개 항목)

...
import { 
  ...
  Receipt, PlusCircle, ArrowLeft, Store, Sun, Moon 
} from 'lucide-react';
...
export function ConsoleSidebar() {
  ...
  const navItems = [
    { path: '/owner/dashboard', label: '실시간 관제 (POS)', icon: LayoutDashboard },
    ...
    <aside className="w-64 bg-white dark:bg-[#09090b] border-r border-neutral-200 dark:border-white/10 flex flex-col justify-between p-5 select-none shrink-0 h-screen sticky top-0 transition-colors duration-300">
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-[#3182f6] to-[#4894fe] flex items-center justify-center shadow-md">
              <Store className="w-5 h-5 text-white" />
            </div>
            <div className="text-left">
              <h2 className="text-base font-black text-neutral-900 dark:text-white leading-tight">ZariYo POS</h2>
              <span className="text-[9px] font-bold font-mono text-[#3182f6] bg-[#3182f6]/10 px-2 py-0.5 rounded-full">KITCHEN CONSOLE</span>
            </div>
          </div>
      ...
      <div className="space-y-2 pt-4 border-t border-neutral-200 dark:border-white/10">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-neutral-200 dark:border-white/10 text-xs font-bold text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-white/5 cursor-pointer transition-all"
        >
          <ChefHat className="w-4 h-4 text-[#3182f6]" />
          테이블 키오스크 모드
        </button>

        <button
          onClick={() => navigate('/')}
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold text-neutral-500 hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-white cursor-pointer transition-all"
        >
          <ArrowLeft className="w-3.5 h-3.5 text-[#3182f6]" />
          메인 화면으로
        </button>
      </div>
